...
def get_rgb_compare(points,img,img_deal,name):
    '''
    比较两张图片某几个点的rgb差值
    :param points: 几个点坐标
    :param img: 原图
    ...
    differences = []
    for [x,y] in points:
        difference = [int(img_deal[x,y,0])-int(img[x,y,0]),int(img_deal[x,y,1])-int(img[x,y,1]),int(img_deal[x,y,2])-int(img[x,y,2])]
        print(name,'->点',[x,y],'三通道差值为:',difference)
...
class Filter(object):
    ...
    def _brightcontrast(self, img):
        return cv2.convertScaleAbs(img, self._alpha, self._beta)
    ...
    def process(self, img):
        whiteimg = self._white_balance(img)
        final = np.hstack((img, whiteimg))
        bcimg = self._brightcontrast(img)
        final = np.hstack((final, bcimg))
        wbcimg = self._brightcontrast(whiteimg)
        final = np.hstack((final, wbcimg))
